...
def apple_signin():
    ...
    try:
        ...
        if not data:
            logger.warning("No data provided in Apple Sign-In request")
            return jsonify({'error': 'No data provided'}), 400
        ...
        logger.info(f"Apple Sign-In attempt - Apple User ID: {apple_user_id}, Email: {email}")

        if not id_token:
            logger.warning("Missing id_token in Apple Sign-In request")
            return jsonify({'error': 'Apple identity token is required'}), 400

        if not apple_user_id:
            logger.warning("Missing apple_user_id in Apple Sign-In request")
            return jsonify({'error': 'Apple user ID is required'}), 400

        # Decode and validate Apple identity token (JWT)
        try:
            import base64
            import json as json_lib
            ...
            token_parts = id_token.split('.')
            if len(token_parts) != 3:
                logger.warning("Invalid Apple ID token format")
            ...
            if token_data.get('iss') not in ['https://appleid.apple.com', 'https://appleid.cdn-apple.com/appleauth/auth']:
                logger.warning(f"Invalid Apple token issuer: {token_data.get('iss')}")
                return jsonify({'error': 'Invalid Apple token'}), 401

            token_sub = token_data.get('sub')
            if token_sub != apple_user_id:
                logger.warning(f"Apple user ID mismatch: token sub={token_sub}, request={apple_user_id}")
                return jsonify({'error': 'Apple user ID mismatch'}), 401

            token_email = token_data.get('email')
            verified_email = (token_email or email or '').strip() or None
            verified_name = (full_name or token_data.get('email') or '').strip() or f"Apple User {apple_user_id[:8]}"

        except Exception as e:
            logger.error(f"Error verifying Apple token: {str(e)}", exc_info=True)
            return jsonify({'error': 'Failed to verify Apple token'}), 401

        # Use placeholder email when Apple does not provide one (e.g. Hide My Email)
        if not verified_email:
            verified_email = f"apple_{apple_user_id}@users.apple.placeholder"
        ...
        try:
            ...
            if user:
                ...
                logger.info(f"Creating new user for Apple Sign-In: {verified_email}")

                base_username = verified_name.lower().replace(' ', '_').replace('.', '_')[:30] if verified_name else f"apple_{apple_user_id[:12]}"
                username = base_username
            ...
            token = generate_jwt_token(user_id, username)
            logger.info(f"Apple Sign-In successful for user: {username} (ID: {user_id})")
            ...
            response_data = {
                ...
                'user': {
                    'id': user_id,
                    'username': username,
                    'email': verified_email if '@' in verified_email and 'placeholder' not in verified_email else (email or ''),
                    'name': verified_name
                }
            }
            response = jsonify(response_data)
            ...
            conn.close()
            logger.error(f"Database integrity error during Apple Sign-In: {str(e)}", exc_info=True)
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute('SELECT * FROM users WHERE email = ?', (verified_email,))
            user = cursor.fetchone()
            ...
            if user:
                ...
                response_data = {
                    ...
                    'user': {
                        'id': user['id'],
                        'username': user['username'],
                        'email': user['email'] if '@' in str(user['email']) and 'placeholder' not in str(user['email']) else (email or ''),
                        'name': verified_name
                    }
                }
                response = jsonify(response_data)
